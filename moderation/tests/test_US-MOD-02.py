...
class TestUSMOD02:
    ...
    def test_next_returns_oldest_pending(self, api_client, db_session, moderator_id):
        ...
        assert str(ticket_old.assigned_moderator_id) == str(moderator_id)
    ...
    @pytest.mark.asyncio
    async def test_concurrent_two_moderators_get_different_cards(self, db_session):
        """
        Две сессии не получают одну карточку.
        Конкурентный тест с asyncio.
        """
        import asyncio
        from uuid import uuid4
        ...
        try:
            clean_session.query(ModerationTask).delete()
            ...
            for i in range(2):
                ticket = ModerationTask(
                    ...
                    status=TaskStatus.PENDING.value,
                    queue_priority=1,
                    json_after={"title": f"Test {i}", "skus": []},
                )
                clean_session.add(ticket)
            clean_session.commit()
            
            tickets = clean_session.query(ModerationTask).filter(
                ModerationTask.seller_id == seller_id
            ).all()
        ...
        async def claim_ticket(moderator_id):
            """Каждый запрос использует свою сессию"""
            # Создаём отдельную сессию для этого запроса
            thread_session = SessionLocal()
            
            # Переопределяем get_db для этого потока
            def override_get_db():
                try:
        ...
        moderator_ids = [uuid4(), uuid4()]
        await asyncio.gather(claim_ticket(moderator_ids[0]), claim_ticket(moderator_ids[1]))
        
        assert len(errors) == 0, f"Errors: {errors}"
        assert len(results) == 2, f"Got {len(results)} tickets, expected 2. Results: {results}"
        assert results[0] != results[1], f"Same ticket: {results}"
        assert set(results) == set(ticket_ids), f"Wrong tickets. Got {results}, expected {ticket_ids}"
